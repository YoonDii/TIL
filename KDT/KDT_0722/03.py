...
'''import os부터 MV_API_KEY는 
내 api키를 다른 사람이 못보게 가리게 만들어줌'''
...
def ranking():
  ...
  lst1 = sorted(info, key= lambda k: k['vote_average'], reverse=True) 
  # lst1은 유명한 영화를 평점 높은 순으로 정렬/  lambda로 가상의함수 설정 /참이면 거꾸로 반환

  lst2 = []                 # list 설정
  for x in range(5):        # x를 5개만 추출
    lst2.append(lst1[x])    # 추출한 x를 lst2에 추가
  return lst2
